...
export const updateGalleryImage = (details) => async (dispatch) =>{
  try {
    ...
    if(data.status_code == 200){
      ...
      toast.success(data?.message)
      // setTimeout(()=>{window.location.href =`/property-list/${data?.gallery.property_id}/gallery`;},1000)
    }else{
      toast.error(data?.message)
    }
  } catch (error) {
...
// add hostel
export const createHostel = details => async (dispatch)=>{
  try {
    dispatch({ type: HOSTEL_ADD_REQUEST });
    const { data } = await API.post(`/create-hostel`, details);
    if (data.status_code == 200) {
      dispatch({ type: HOSTEL_ADD_SUCCESS, payload: data?.hostel });
      toast.success(data?.message)
    } else {
      toast.error(data?.message)
    }
...
export const getHostel = () => async (dispatch) => {
  try {
    dispatch({ type: GET_HOSTEL_LIST });
    const { data } = await API.get(`/get-hostel`);
    dispatch({ type: SUCCESS_HOSTEL_LIST, payload: data });
  } catch (error) {
    dispatch({
      type: FAILURE_HOSTEL_LIST
    });
    toast.error(error?.message)
  }
};
...
//update hostel
export const updateHostel = (details) => async (dispatch) => {
  try {
    dispatch({ type: HOSTEL_UPDATE_REQUEST });
    const { data } = await API.put(`/update-hostel`, details);
    if (data.status_code == 200) {
      dispatch({ type: HOSTEL_UPDATE_SUCCESS, payload: data?.hostel });
      toast.success(data?.message)
    } else {
      toast.error(data?.message)
    }
