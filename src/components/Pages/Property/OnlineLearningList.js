...
export default function OnlineLearning() {
  ...
  const { users, college, tab_status } = useSelector(state => ({
    ...
    college: state?.propertyType?.college.filter(item => item?.edu_type == "OnlineLearning"),
  ...
  return (
    <div>
      <div className="page-header">
        <div>
          <h1 className="page-title"> Online Learning Property List</h1>
          <Breadcrumb className="breadcrumb">
            <Breadcrumb.Item className="breadcrumb-item" href="#">
              Property
            </Breadcrumb.Item>
            <Breadcrumb.Item className="breadcrumb-item active breadcrumds" aria-current="page">
            Online Learning Property List
            </Breadcrumb.Item>
          </Breadcrumb>
        </div>
      ...
      <Row className=" row-sm">
        <Col lg={12}>
          <Card>
            <Card.Header>
              <h3 className="card-title">Online Learning Property List</h3>
            </Card.Header>
            <Card.Body>
