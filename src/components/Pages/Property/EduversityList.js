...
export default function Eduversity() {
    const dispatch = useDispatch();
    const { users, college, tab_status } = useSelector(state => ({
        users: state?.userAuth?.loginUser?.user,
        college: state?.propertyType?.college.filter(item => item?.edu_type == "Eduversity"),
    ...
    return (
        <div>
            <div className="page-header">
                <div>
                    <h1 className="page-title"> Eduversity Property List</h1>
                    <Breadcrumb className="breadcrumb">
                        <Breadcrumb.Item className="breadcrumb-item" href="#">
                            Property
                        </Breadcrumb.Item>
                        <Breadcrumb.Item className="breadcrumb-item active breadcrumds" aria-current="page">
                            Eduversity Property List
                        </Breadcrumb.Item>
                    </Breadcrumb>
                </div>
            ...
            <Row className=" row-sm">
                <Col lg={12}>
                    <Card>
                        <Card.Header>
                            <h3 className="card-title">Eduversity Property List</h3>
                        </Card.Header>
                        <Card.Body>
